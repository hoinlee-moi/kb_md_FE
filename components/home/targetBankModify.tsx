...
export default function TargetBankModify({ setIsOpen, name, targetAmount, refet }: PropsType) {
  const submitHandler = async (formData: FormData) => {
    const target = formData.get("target") as string;
    const targetMoney = formData.get("targetMoney") as string;
    // try {
    //   const res = await updateSavingGoal({ goalId: 0, name: target, targetAmount: +targetMoney });
    //   if (res.ok) {
    //     setIsOpen(false);
    //     refet();
    //   }
