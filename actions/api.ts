...
export const getNationalRanking = async (): Promise<GetRankResponDate> => {
  const res = await fetch(`${URL}/api/rankings/nationwide/${userId}`, { cache: "no-store" });
...
export const getRegionalRanking = async (): Promise<GetRankResponDate> => {
  const res = await fetch(`${URL}/api/rankings/region/${userId}`, { cache: "no-store" });
...
export const getUserKBScore = async (): Promise<number> => {
  const res = await fetch(`${URL}/api/rankings/score/${userId}`, { cache: "no-store" });
...
// 저축 목표 입금
export const depositToSavingGoal = async (goalId: number) => {
...
export const getSavingGoalInfo = async (): Promise<GetSavingGoalInfo[]> => {
  const res = await fetch(`${URL}/api/saving-goals/inform/${userId}`, { cache: "no-store" });
...
// 당월 지출 수입 합계 조회
export const getMonthlyIncomeAndExpense = async (month: number): Promise<GetMonthlyExTotal> => {
  const res = await fetch(`${URL}/api/transactions/monthly-summary/${userId}/${month}`, { cache: "no-store" });
...
export const getRecentTransactions = async (): Promise<GetRecentTrans[]> => {
  const res = await fetch(`${URL}/api/transactions/recent/${userId}`, { cache: "no-store" });
...
export const getTotalAccountBalance = async (): Promise<GetTotalAccBal> => {
  const res = await fetch(`${URL}/api/transactions/accounts/total-balance/${userId}`, { cache: "no-store" });
